    ...
    with open(filename, "w", encoding="utf-8", newline="") as f:
        for _ in range(num_records):
            ...
            record = (
                ...
                f"{navn:<21}"      # Navn (20 tegn)
                f"{fødselsdato:<10}"    # Fødselsdato (10 tegn)
                f"{adresse:<31}"        # Adresse (30 tegn)
    ...
    with open(filename, "w", encoding="utf-8", newline="") as f:
        for _ in range(num_records):
            ...
            record = (
                ...
                f"{navn:<21}"          # Navn (20 tegn)
                f"{alias_list[0]:<21}" # Alias1 (20 tegn)
                f"{alias_list[1]:<21}" # Alias2 (20 tegn)
                f"{alias_list[2]:<21}" # Alias3 (20 tegn)
                f"{alias_list[3]:<21}" # Alias4 (20 tegn)
                f"{alias_list[4]:<21}" # Alias5 (20 tegn)
                f"{fødselsdato:<10}"   # Fødselsdato (10 tegn)
                f"{land:<2}"           # Land (2 tegn)
                "\n"
            )
